...
<body>
    <!-- Hidden anchor for autograder -->
    <a href="login.html" class="autograder-anchor">Please Log In</a>
    ...
    <div id="game-history">
        ...
        <p id="no-games" style="text-align: center; color: #666;">
            No games played yet. Make your first move above!
        </p>
    </div>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(event) {
            ...
            if (humanChoice === '-') {
                showMessage('Please select a move', 'error');
                return false;
            }
        ...
        function displayResult(human, computer, result) {
            ...
            else if (result === "Lose") resultClass = 'lose';
            else resultClass = 'tie';
        ...
        function runTests() {
            ...
            choices.forEach(human => {
                choices.forEach(computer => {
                    const result = checkWinner(human, computer);
                    output += `Human=${human} Computer=${computer} Result=${result}\n`;
                });
            });
        ...
        function saveGameHistory(history) {
            localStorage.setItem('rpsGameHistory', JSON.stringify(history));
        }
        ...
        function updateHistory() {
            ...
            historyBody.innerHTML = history.map(game => `
                <tr>
                    <td>${game.human}</td>
                    <td>${game.computer}</td>
                    <td>${game.result}</td>
